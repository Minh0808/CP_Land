...
const Searchbar: React.FC<SearchbarProps> = ({ onSearch }) => {
   ...
   useEffect(() => {
      ...
      if (provFull && provFull['quan-huyen']) {
         ...
         const districtArray = Object.values(provFull['quan-huyen']).map((distObj: any) => ({
            code: distObj.code,
            name: distObj.name,
         })) as DistrictItem[];
   ...
   useEffect(() => {
      ...
      const distFull = (provFull['quan-huyen'] as Record<string, any>)[selectedDistrict];
      if (distFull && distFull['xa-phuong']) {
         const wardArray = Object.values(distFull['xa-phuong']).map((wardObj: any) => ({
            code: wardObj.code,
            name: wardObj.name,
         })) as WardItem[];
            ...
            <SubmitContainer>
               ...
               <Selected
                  ...
                  onChange={(e) => {
                     ...
                     // nếu là sort theo giá
                     if (v === 'price-asc' || v === 'price-desc') {
                        // gọi onSearch với trường sortOrder
                        onSearch?.({
                           keyword: keyword.trim(),
                           provinceCode: selectedProvince,
                           districtCode: selectedDistrict,
                           wardCode: selectedWard,
                           propertyType: type, // giữ nguyên loại hình (nếu đã chọn trước)
                           sortOrder: v,
                        });
                        // nếu bạn cũng lưu local state để highlight select
                        setSortOrder(v);
                     } else {
                        // ngược lại, là lọc theo propertyType
                        onSearch?.({
                           keyword: keyword.trim(),
                           provinceCode: selectedProvince,
                           districtCode: selectedDistrict,
               ...
               >
                  <option value="menu_order">Lọc</option>
